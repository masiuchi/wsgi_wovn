    ...
    def is_valid_settings(self):
        ...
        if 'user_token' not in settings \
                ...
                or len(settings['user_token']) > 6:
                    valid = False
                    errors.append('User token is %s not valid.' % settings['user_token'])

        if 'secret_key' not in settings or not settings['secret_key']:
            valid = False
            errors.append('Secret key %s is not valid.' % settings['secret_key'])

        if 'url_pattern' not in settings or not settings['url_pattern']:
            valid = False
            errors.append('Url pattern %s is not valid.' % settings['url_pattern'])
        ...
        if 'default_lang' not in settings or not settings['default_lang']:
            valid = False
            errors.append('Default lang %s is not valid.' % settings['default_lang'])
    ...
    def settings(self, opts={}):
        ...
        if not self.__config_loaded:
            ...
            if 'supported_langs' not in self.__settings:
                self.__settings['supported_langs'] = [self.__settings['default_lang']]
            ...
            if self.__settings.get('url_pattern') == 'path':
                ...
                self.__settings['url_pattern_reg'] = r'((\?.*&)|\?)wovn=(?P<lang>[^&]+)(&|$)'
